...
def show_compare_menu(root):
    ...
    def run_and_show(kind):
        ...
        if kind == "uninformed":
            ...
            labels=["Backtracking","FB"]; funcs=[_backtracking_bench,_fb_bench]
            times=[]; nodes=[]
            for f in funcs:
                t,n,_=f(target_idx); times.append(t*1000.0); nodes.append(n)
            fig,(ax1,ax2)=plt.subplots(2,1,figsize=(8,6)); fig.suptitle("So sánh - Backtracking")
            b=ax1.bar(labels,times,color="#B0CF6B"); ax1.set_title("Thời gian thực thi"); ax1.set_ylabel("Milli-giây")
            [ax1.text(x.get_x()+x.get_width()/2,x.get_height(),f"{v:.2f}",ha='center',va='bottom') for x,v in zip(b,times)]
            c=ax2.bar(labels,nodes,color="#89CF6B"); ax2.set_title("Số node mở rộng"); ax2.set_ylabel("Số node")
            [ax2.text(x.get_x()+x.get_width()/2,x.get_height(),f"{v}",ha='center',va='bottom') for x,v in zip(c,nodes)]
            fig.tight_layout(rect=[0,0.03,1,0.95]); _save_fig(fig,'backtracking_compare.png'); _render_fig_in_tk(root, fig, "Backtracking")
